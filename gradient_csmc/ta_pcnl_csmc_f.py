"""
Implements the twisted Particle-aPCNL kernel of the paper.
"""
...
def get_kernel(mu0: Array, P0: Array, r0: Callable,
    ...
    """
    Constructor for the twisted Particle-aPCNL kernel.
    ...
    def kernel(key: PRNGKey, x_star: Array, b_star: Array, ells: Array, deltas: Array):
        """
        Implements the twisted Particle-aPCNL kernel of the paper for pre-computed reversed dynamics.
        ...
        drift = 0.5 * deltas[:, None] * jax.vmap(lambda C, g: C @ g)(
            jnp.insert(Qs, 0, P0, axis=0), 
            grad_log_w_star
        )
 
        eps_aux = jax.random.normal(key_aux, shape=(T, d_x))
        noise = jax.vmap(lambda delta, chol, eps: jnp.sqrt(0.5 * delta) * (chol @ eps))(
            deltas, 
            jnp.insert(chol_Qs, 0, chol_P0, axis=0),
            eps_aux
        )
        ...
        ### Kalman stuff
        # Marginal posteriors
        rev_Rs = (0.5 * ells)[:, None, None] * jnp.insert(Qs, 0, P0, axis=0)
        ...
        Mt_params = As, cs, chol_Es, inv_chol_Es, Qs, inv_chol_Qs
...
def Mt_logpdf(x_prev, x, params):
    A, c, _, inv_cholE, *_ = params
    mean_t = x_prev @ A.T + c[None, :]
    return mvn_logpdf(x, mean_t, None, inv_cholE, constant=False)


def Gamma_0(x, val_and_grad_r0, params):
    u, prop_m0, prop_inv_chol_P0, P0, chol_P0_inv, delta, ell = params
    r0_val, r0_grad = val_and_grad_r0(x)
    out = r0_val
    correction_term = mvn_logpdf(x + 0.5 * delta * (r0_grad @ P0.T), u, None, chol_inv=jnp.sqrt(2.0 / ell)*chol_P0_inv, constant=False)
    correction_term -= mvn_logpdf(x, u, None, chol_inv=jnp.sqrt(2.0 / ell)*chol_P0_inv, constant=False)
...
def Gamma_t(x_prev, x, params, val_and_grad_rt):
    ...
    A, c, chol_E, inv_chol_E, Q, chol_Q_inv = Mt_params_here

    rt_val, rt_grad = val_and_grad_rt(x_prev, x, rt_params_here)
    out = rt_val
    correction_term = mvn_logpdf(x + 0.5 * delta * (rt_grad @ Q.T), u, None, chol_inv=jnp.sqrt(2.0 / ell)*chol_Q_inv, constant=False)
    correction_term -= mvn_logpdf(x, u, None, chol_inv=jnp.sqrt(2.0 / ell)*chol_Q_inv, constant=False)
