"""
No-op progress bar for JAX scans.

We disable all host-side callbacks (tqdm, printing) because they are not
supported with modern JAX + PJRT and cause errors when used inside vmap/scan.
"""

def progress_bar_scan(num_samples, message=None, show=None):
    """Return a decorator that leaves the scan body unchanged (no progress bar)."""
    def _decorator(body_fun):
        return body_fun
    return _decorator
...
#         _ = lax.cond(
#             iter_num == 0,
#             lambda _: callback(_define_tqdm, None, result=iter_num),
#             lambda _: iter_num,
#             operand=None,
#         )
...
#             lambda _: callback(_update_tqdm, (print_rate, carry_show), result=iter_num),
#             lambda _: iter_num,
#             operand=None,
#         )
...
#             lambda _: callback(_update_tqdm, (remainder, carry_show), result=iter_num),
#             lambda _: iter_num,
#             operand=None,
#         )
...
#             lambda _: callback(_close_tqdm, None, result=result),
#             lambda _: result,
#             operand=None,
#         )
