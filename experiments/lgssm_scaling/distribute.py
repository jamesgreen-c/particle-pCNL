...
#     if entry[-1][1] == "twisted":
#         print(i)

# loop over if no i provided
args = parser.parse_args()
if args.i == -1:
    for i in range(len(combination)):
        log_var, T, d, (kernel, style, scale, delta_arg) = combination[i]
        exec_str = "python3 experiment.py --delta-scale {} --delta-arg {} --T {} --kernel {} --style {} --D {} --N 31 --log-var {} --verbose --M 1000"
    ...
    exec_str = "python3 experiment.py --delta-scale {} --delta-arg {} --T {} --kernel {} --style {} --D {} --N 31 --log-var {} --verbose --M 1000"
    print(exec_str.format(scale, delta_arg, T, kernel.value, style, d, log_var))
    os.system(exec_str.format(scale, delta_arg, T, kernel.value, style, d, log_var))
